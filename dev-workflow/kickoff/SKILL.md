...
3. **Determine the target repo (rooting).** Map the task to its repo so the new
   ...
   the user if it's not there or you're unsure, and do not trust a stale
   entry - verify it exists before rooting to it. For work not tied to a
   specific repo (research, planning, misc tasks), the general hub is fine.
...
6. **Write the brief.** Self-contained (the new session has none of this
   ...
   **If the brief carries a design artifact** - a mock, a diagram, a spec page,
   anything iterated with the user *before* the work started - three things are
   mandatory. Skipping them is how an artifact agreed over several rounds gets
   built into something that does not resemble it, with nobody at fault.

   - **Split its properties into BINDING and INCIDENTAL, as two explicit lists.**
     An artifact drawn from scratch carries its own colours, spacing and idiom,
     which the target codebase should usually *not* inherit - and it also
     carries the one or two properties it existed to communicate. A blanket
     "this is direction, not specification" is not neutral: it discards the
     second along with the first. Whatever your summary drops is silently not
     built, and the building agent cannot notice, because it never sees the
     artifact.
   - **Hand over the artifact as a file path the agent can open** - not a link
     it cannot reach, not a paraphrase - and tell it to keep it open and compare
     against it while working. Two real costs come with that. The artifact must
     stay out of the repo if it holds anything unpublishable, and its content
     now sits in the agent's context while that agent writes repo documentation.
     A pre-push privacy check in the receiving repo is the control that catches
     the second; a sentence in the brief is not.
   - **Require a fixture that exercises every block the result can show**,
     handed back as something openable. Not a screenshot: a screenshot proves
     nothing was inspected, and a broken page photographs fine. The fixture is
     the test and the openable artifact is what makes it reviewable - a result
     rendered with one token row proves nothing about a design whose whole
     argument is proportion.

   Add a **report-rather-than-decide** clause either way: anything that does not
   fit the binding list is a finding to send back, not a licence to improvise.
